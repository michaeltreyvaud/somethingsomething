...
import CategoryDelete from './Delete/delete.container';
...
class Category extends React.Component {
  ...
  componentDidMount() {
    const { listsafetyRecordCategoryCategory } = this.props;
  ...
  render() {
    ...
    const simpleButtons = (item, index) => [
      ...
      switch (key) {
        case 1: {
          onClick = () => history.push(`/dashboard/safety/category/${item.id}`);
          break;
        }
        case 2: {
    ...
    const tableData = items.map((_item, index) => {
      const item = [_item.name, simpleButtons(_item, index)];
      return item;
    });
    return (
      <div>
            ...
            <Card>
              <CardHeader color="rose" icon>
                ...
                <Button
                  ...
                  onClick={() => history.push('/dashboard/safety/category/create')}
                >
                  Create
                </Button>
              </CardHeader>
...
export default withRouter(withStyles(style)(Category));

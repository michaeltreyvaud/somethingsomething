...
class Cooling extends Component {
  ...
  render() {
    ...
    const { refrigerationStartTime, temperature, comments } = record;
        ...
        {selectedFoodItem !== '' && (
          <div>
            <InputLabel className={classes.label}>
              Time Into Refrigeration
            </InputLabel>
            <FormControl fullWidth>
              <Datetime
                value={moment(refrigerationStartTime || '')}
                dateFormat="DD/MM/YYYY"
                inputProps={{ placeholder: 'Select Time' }}
                onChange={date => setRecordValue('refrigerationStartTime', date.valueOf())}
              />
            </FormControl>
            <CustomInput
              value={temperature}
...
export default withStyles(extendedFormsStyle)(Cooling);

...
export const createServices = services => async (dispatch) => {
  try {
    ...
    const body = services;
    //  TODO - fetch these
    const { REACT_APP_API_URL, REACT_APP_CREATE_SERVICES_PATH } = process.env;
    const response = await AuthenticatedFetch(`${REACT_APP_API_URL}${REACT_APP_CREATE_SERVICES_PATH}`, body);

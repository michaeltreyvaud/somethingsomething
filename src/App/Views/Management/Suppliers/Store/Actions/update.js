...
export const updateSupplier = supplier => async (dispatch) => {
  try {
    ...
    const body = supplier;
    //  TODO - fetch these
    const { REACT_APP_API_URL, REACT_APP_MANAGEMENT_SUPPLIER_UPDATE_PATH } = process.env;
    const updatedItem = await AuthenticatedFetch(`${REACT_APP_API_URL}${REACT_APP_MANAGEMENT_SUPPLIER_UPDATE_PATH}`, body);
